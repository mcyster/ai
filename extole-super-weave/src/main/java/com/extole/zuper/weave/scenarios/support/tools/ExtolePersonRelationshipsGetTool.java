...
import com.extole.zuper.weave.scenarios.support.tools.ExtolePersonRelationshipsGetTool.Request;
...
class ExtolePersonRelationshipsGetTool implements ExtoleSupportTool<Request> {
    ...
    public Object execute(Request request, ExtoleSuperContext context, Weave weave) throws ToolException {
        ...
        try {
            result = this.extoleWebClientFactory
                    .getWebClientById(request.client_id).get().uri(uriBuilder -> uriBuilder
                            .path("/v4/runtime-persons/" + request.person_id + "/relationships").build())
