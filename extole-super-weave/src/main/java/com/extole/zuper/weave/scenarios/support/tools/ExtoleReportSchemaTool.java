...
public class ExtoleReportSchemaTool implements ExtoleSupportTool<Request> {
    ...
    private JsonSchema getReportSchema(String clientId, String reportId, ReportRunType runType) throws ToolException {
        ...
        try {
            System.out.println("dataNodes: "
                    + (new ObjectMapper()).writerWithDefaultPrettyPrinter().writeValueAsString(dataNodes));
    ...
    private JsonNode tryHardToloadReport(String clientId, String reportId, ReportRunType runType, int limit)
    ...
    private Optional<JsonNode> loadReport(String clientId, String reportId, ReportRunType runType, int limit)
            throws ToolException {
        ...
        try {
            response = this.extoleWebClientFactory.getWebClientById(clientId).get()
                    .uri(uriBuilder -> uriBuilder.path(url).queryParam("limit", String.valueOf(limit)).build())
    ...
    public enum ReportRunType {
        ...
        REPORT, @JsonProperty("ReportRunner")
    ...
    public static record Request(
            ...
            @JsonPropertyDescription("The 20 to 22 character alphanumeric Extole report id") @JsonProperty(required = true) String reportId,

            @JsonPropertyDescription("Report type") @JsonProperty(required = false) ReportRunType reportRunType) {
        public Request(@JsonProperty("clientId") String clientId, @JsonProperty("reportId") String reportId,
