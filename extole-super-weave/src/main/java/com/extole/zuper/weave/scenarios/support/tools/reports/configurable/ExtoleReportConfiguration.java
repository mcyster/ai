...
public class ExtoleReportConfiguration implements ExtoleSupportAdvisorToolLoader {
    ...
    public ExtoleReportConfiguration(ExtoleTrustedWebClientFactory extoleWebClientFactory, ApplicationContext context)
            throws IOException, ExtoleReportConfigurtationException {
            ...
            for (Resource resource : resources) {
                ...
                try (InputStream inputStream = resource.getInputStream()) {
                    ...
                    if (!name.matches("[a-zA-Z0-9]+")) {
                        throw new ExtoleReportConfigurtationException(
                                "Runbook name must only contain alphanumeric characters: " + name, resource);
                    }

                    ExtoleConfigurableTimeRangeReportTool.Configuration configuration;
                    try {
                        configuration = mapper.readValue(inputStream,
                                ExtoleConfigurableTimeRangeReportTool.Configuration.class);
                    ...
                    var reportTool = new ExtoleConfigurableTimeRangeReportTool(name, configuration,
                    ...
                    logger.error(
                            "Failed to load resource as a ExtoleConfigurableTimeRangeReportTool.Configuration from "
    ...
    private static String capitalize(String input) {
        ...
        for (String word : words) {
            if (!word.isEmpty()) {
                capitalizedWords.append(word.substring(0, 1).toUpperCase()).append(word.substring(1).toLowerCase());
            }
        }
